...
4. Diff against `eval/datasets/e2e_sweep_results.md` — its own recorded current result (check the date/count in the file itself, don't assume a number; as of the last `phase-2` update it's 47/50, superseding the older 44/50 Week 8/16 figure) — not the unrelated 56/60 frozen production number, a different harness with a different denominator.
