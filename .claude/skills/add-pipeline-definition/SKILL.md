...
3. If the target agent needs a shared constant for its `SharedData` key (like `PipelineDefinitions.TargetRecipeKey` for diet validation), define it once in `PipelineDefinitions` and reference it from both the pipeline definition and the agent's `HandleAsync` — don't let the same string literal drift across multiple files (this is exactly what `phase2-week2-progress.md` describes as already having happened once, labeled `P2-5` in that doc — note that label is **not** in `docs/tech-debt.md`; verified that file has no `P2-`-prefixed entries at all).
